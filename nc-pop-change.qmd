...
In @fig-estimates-state-rates-natural-migration each thin line shows one county's rate of natural increase or rate of net migration. The thick straight lines are linear regression lines (not weighted by county population).
...
multi_year_nc_counties |>
  ...
  geom_line(aes(group = county), alpha = 0.1) +
  geom_smooth(method = "lm", formula = 'y ~ x', se = FALSE) + #, fill = "blue") +
  ...
  labs(
    title = glue("NC population increase 2015-2019 was due to net migration",
                 "\nRates of natural increase and net migration"),
    subtitle = glue("Birth and death rates were nearly equal; death rate overtook birth rate during this time period"),
    x = NULL,
    y = "Yearly rate",
    color = NULL,
    caption = my_caption
...
At the state-level, in this time period the pattern repeats each year: there are more births than deaths, and domestic and international migration combine to nearly cancel out the deaths, resulting in a population growth of about one percent each year.
