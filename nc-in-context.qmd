...
Regional designations below approximately match those in Michael L. Walden's *North Carolina beyond the connected age: the Tar Heel State in 2050* with these differences: 

1. I followed county boundaries whereas he drew ellipses, which greatly simplifies my work.
...
Nighttime light (radiance) is a good proxy for population density.^[[Exploring Nighttime Light in the U.S. Southeast: Model specification and evaluation in the tidymodels framework](https://dmoul.github.io/nightlight/introduction.html) by Daniel Moul ] The North Carolina urban crescent is particularly noticeable. Also noteworthy: the lights reveal the same pattern in North Carolina as in other states in the southeast: urban areas extend across county lines, and nearly every rural county has one at least town with higher density population than the surrounding area--presumably the county seat.
...
```{r}
# debug: does the radiance ecdf look OK? Yes.
rad <- tibble(radiance =  values(light_map_world_part_projected)) |>
  filter(!is.nan(radiance),
         !is.na(radiance)) |>
  slice_sample(prop = 0.025)

ggplot(rad) +
  stat_ecdf(aes(radiance+1),
            na.rm = TRUE) +
  scale_x_log10()
```
